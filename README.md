...
研究室での流体解析作業において、以下の課題がありました。

- OpenFOAM はコマンドラインが中心で、**作業手順が属人化**しやすい
- メンバー交代のたびに「クーラン数はいくつにすれば良いか」といった **ノウハウが失われる**
- AI が生成した設定ファイルやスクリプトを、**安全性を確認せずに実行してしまう** リスク

これらを解決するため、**Python / Streamlit** でブラウザから操作できる統合 GUI ツールを自主開発しました。
