# 🌊 OpenFOAM Knowledge Platform

> 流体解析ソフト **OpenFOAM** の操作を GUI で完結させ、組織のノウハウを共有するための統合プラットフォーム

[![Python](https://img.shields.io/badge/Python-3.10%2B-blue?logo=python)](https://www.python.org/)
[![Streamlit](https://img.shields.io/badge/Streamlit-1.35%2B-red?logo=streamlit)](https://streamlit.io/)
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)

---

### ⚙️ Setup — ケースパラメータ設定
`config.json` から読み込んだデフォルト値を初期値として各入力欄に表示。レイノルズ数・代表速度・動粘性係数から Re を自動計算し、乱流モデルや並列コア数を選択できる。

![Setup画面1](docs/screenshot_setup_1.png)
![Setup画面2](docs/screenshot_setup_2.png)

---

### 🚀 Run & Monitor — コマンド実行 & ナレッジパネル
左カラムのボタンを押すと WSL 経由で OpenFOAM コマンドを実行。右カラムには `knowledge.json` から読み込んだそのコマンド固有のノウハウ・注意事項をリアルタイムで表示する。

![Run & Monitor画面](docs/screenshot_run.png)

---

### 🔍 Troubleshoot — エラーログ解析
ターミナルのエラーログを貼り付けてボタンを押すと、`knowledge.json` に定義したキーワードパターンとマッチングし、原因と対処法を提示する。Run 画面で保存された実行ログをワンクリックで読み込むことも可能。

![Troubleshoot画面](docs/screenshot_troubleshoot.png)

---

### ✅ AI Code QA — AI生成コード 安全性検証チェックリスト
AI が生成したスクリプトや設定ファイルを実運用する前の「関所」。5項目すべてにチェックが入ると `st.success` で実行許可が表示される。各項目には折りたたみ式の詳細確認手順を配置。

![AI Code QA画面1](docs/screenshot_qa_1.png)
![AI Code QA画面2](docs/screenshot_qa_2.png)

---

## 🎯 開発背景と狙い

### 課題：Scrapbox による知識のサイロ化

研究室ではこれまで、解析ノウハウの記録に Scrapbox を活用してきました。個々の作業メモとしては有効に機能していたものの、「他のメンバーのページを積極的に読みに行く機会が少ない」という課題がありました。その結果、実践的な解析手法の共有が進まず、知識が個人の中に閉じてしまう、いわゆる属人化が生じていました。

### 解決策：「読む共有」から「実行に溶け込む共有」へ

この問題を解決するために、「記録を読むことで共有する」という受動的な方法から、「解析を実行する過程そのものにノウハウを組み込む」という新しいアプローチを目指しました。そこで、OpenFOAM を利用する研究室メンバー向けに、解析作業の中で自然と知識が共有されるプラットフォームを開発しました。

### 最大の工夫：研究室独自の知見を「あとから登録できる」設計にする

本ツールの核心は、単なる実行用 GUI を作ったことではなく、**研究室独自の知見やルールをシステムに組み込めるようにした点** にあります。

その仕組みが **「検証チェックリスト（AI Code QA）」** です。AI が生成したコードを実行する前に、研究室で標準とする評価軸・用語定義・物理現象の解釈に沿っているかを人間が確認する「関所」として機能します。

本リポジトリは公開用のため、チェック項目には **筆者自身の研究分野（翼まわりの流体解析）を題材にしたサンプル定義** を入れています。例えば次のような項目です。

- 最適化の目的変数を **「揚抗比」** に統一しているか（単なる揚力係数になっていないか）
- 壁面付近の流体挙動を **「付着ジェット」「剥離ジェット」** と区別して扱っているか
- 干渉パターンを **「擬似翼モード（Step 1 / Step 2）」** として分類しているか

> ⚠️ 上記はあくまで **機能を説明するためのサンプル** であり、特定の研究室で確定した取り決めを記載したものではありません。
> 実運用では、各チームが自分たちの評価基準や用語定義を `app.py` 内の `_QA_CHECKS`（将来的には `knowledge.json`）に登録して使う想定です。

このように **「定義そのもの」をコードから分離し、後から差し替えられる設計** にすることで、メンバー間で評価軸や用語のズレが生じない環境を、どの研究テーマにも応用できる形で実現しました。

### 成果と今後の展望

直感的なフロントエンドを導入したことで、コマンドライン操作やプログラミング経験が浅いメンバーでも容易に解析に取り組めるようになりました。その結果、ツールを介したノウハウ共有が促進され、研究室全体の生産性向上に寄与しています。

今後はさらなる機能拡張を進め、データの一貫性の向上と解析スピードのさらなる高速化を目指していきます。

---

## 🛠 技術スタック・実装のポイント

| 技術 | ひと言でいうと | 用途 |
|------|--------------|------|
| **Streamlit** | Pythonだけで画面を作れる道具 | Web GUI フレームワーク（サーバー不要・ローカル完結） |
| **subprocess + WSL 連携** | Windowsから裏でLinuxの計算コマンドを動かす | OpenFOAM コマンドを安全に実行。失敗したら処理を止めてエラー表示する安全装置を実装 |
| **正規表現ログパーサ** | 計算ログの文字列から必要な数値だけ拾う | `pimpleFoam` の出力をリアルタイムで読み取り、残差値（Ux, Uy, p, k, omega）を抽出・蓄積 |
| **matplotlib (Agg)** | 計算結果をグラフ・画像にする | 流れ場の断面データを軽い2Dカラー画像に変換。毎回メモリを解放してPCが重くならないよう配慮 |
| **pandas** | 表形式のデータを扱うライブラリ | 残差の時系列データを管理し、対数軸グラフで可視化 |
| **JSON 外部設定** | 設定を別ファイルに切り出す | パラメータ・ノウハウを `config.json` / `knowledge.json` で管理し、本体コードに直接書かない |

### 設計上のこだわり

専門用語だけだと伝わりにくいので、それぞれ「ひと言でいうと何をしたのか」を添えます。

- **メモリ安全設計**
  *ひと言でいうと*：重い3Dデータを全部読み込むとPCが固まってしまうので、軽い2D断面の画像だけを表示するようにしました。
  *技術的には*：全体メッシュは Python 側で読み込まず、`postProcessing/surfaces/` の2D断面データのみ使用。プレビュー更新も `step % N == 0` の間引き制御で負荷を抑えています。

- **WSL パス変換**
  *ひと言でいうと*：WindowsとLinux（WSL）ではフォルダの書き方が違う（`C:\foo` と `/mnt/c/foo`）ため、これを自動で変換する処理を入れました。
  *技術的には*：`PureWindowsPath` を用いて `C:\foo\bar` → `/mnt/c/foo/bar` に変換し、WindowsアプリからWSL上のOpenFOAMをシームレスに操作できるようにしています。

- **機密データ分離**
  *ひと言でいうと*：研究室の秘密情報をコードに直接書かず、別ファイルに分けて、GitHubには上げないようにしました。
  *技術的には*：固有のパラメータ・ノウハウは `*.json` で外部管理し、`app.py` 本体にはハードコードしない設計。`.gitignore` で実データ用ファイルを Git 管理から除外しています。

---

## 🗺 開発ロードマップ

| バージョン | 内容 | 状態 |
|-----------|------|------|
| **v0.1** | UI モックアップ・JSON 読み込み・レイアウト | ✅ 完了 |
| **v0.2** | `subprocess` による WSL 連携コマンド実行・エラーハンドリング | ✅ 完了 |
| **v0.3** | 残差リアルタイム監視・流場スライス画像プレビュー | ✅ 完了 |
| **v1.0** | AI生成コード安全性検証チェックリスト画面（初回安定版） | ✅ 完了 |
| **v1.1** | ケーステンプレート自動生成・実揚抗力グラフ読み込み | 🚧 継続中 |

---

## 🚀 セットアップ・起動方法

### 前提条件
- Windows 11 + WSL2（Ubuntu 22.04 推奨）
- WSL 側に OpenFOAM がインストール済み
- Python 3.10 以上

### インストール

```bash
git clone https://github.com/<your-username>/openfoam-knowledge-platform.git
cd openfoam-knowledge-platform
pip install -r requirements.txt
```

### 起動

```bash
streamlit run app.py
```

ブラウザで `http://localhost:8501` が開きます。

---

## 📁 ファイル構成

```
.
├── app.py              # メインアプリ（全機能・約1600行）
├── config.json         # デフォルトパラメータ（ダミー値・公開用）
├── knowledge.json      # コマンドノウハウ・トラブルパターン（ダミー値・公開用）
├── requirements.txt    # 依存ライブラリ
├── docs/               # README 用スクリーンショット
└── README.md
```

> **セキュリティ方針**：
> 実運用時の機密パラメータは `config.local.json` / `knowledge.local.json` に保存し、
> `.gitignore` によって Git 管理対象から除外しています。

---

## 📄 ライセンス

MIT License — 詳細は [LICENSE](LICENSE) を参照してください。
